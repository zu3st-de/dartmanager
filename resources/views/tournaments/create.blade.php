<x-app-layout>
            ...
            <div class="bg-gray-900 shadow rounded-xl p-6">
                <form method="POST" action="{{ route('tournaments.store') }}">
                    ...
                    <div id="group-settings" class="space-y-4 hidden">
                        ...
                        <div>
                            <label class="block text-sm text-gray-300">
                                Best of Gruppenphase
                            </label>
                            <select name="group_best_of"
                                class="w-full bg-gray-700 text-white rounded px-3 py-2 border-gray-600 focus:ring-emerald-500">
                                @foreach ([1, 3, 5, 7] as $value)
                                    <option value="{{ $value }}" {{ (int) old('group_best_of', 1) === $value ? 'selected' : '' }}>
                                        Bo{{ $value }}
                                    </option>
                                @endforeach
                            </select>
    ...
    <script>
        ...
        const groupInputs = groupSettings.querySelectorAll('input, select');
